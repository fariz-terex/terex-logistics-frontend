...
function Dashboard({ role, setPage, deliveries, returns, reconciliations, materials }) {
  ...
  const inProgress = deliveries.filter((d) => ["In Progress", "Approved", "Preparing", "Shipped"].includes(d.status)).length;
...
function DeliveryDetail({ delivery, onBack, onApprove, onReject, onAssignStock, onAdvance, role, materials, api }) {
  ...
  const canAssignStock = (role === ROLES.LOGISTICS || role === ROLES.MANAGER) && delivery.status === "Approved";
...
export default function App() {
  ...
  const notifications = useMemo(() => {
    ...
    if (isLogisticsView) {
      deliveries.filter((d) => d.status === "Approved").forEach((d) => {
