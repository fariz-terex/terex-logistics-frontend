...
function Dashboard({ role, setPage, deliveries, returns, reconciliations, materials }) {
  ...
  const inProgress = deliveries.filter((d) => ["In Progress", "Preparing", "Shipped"].includes(d.status)).length;
...
function DeliveryList({ deliveries, setSelected, setPage, role }) {
  ...
  const filtered = filter === "All" ? deliveries : deliveries.filter((d) => d.status === filter || (filter === "In Progress" && ["In Progress", "Preparing", "Shipped", "Approved"].includes(d.status)));
      ...
      {(canApprove || canAdvance) && (
        <Card className="p-5 flex items-center justify-between">
          <div className="text-sm text-gray-600">
            {canApprove && "Tinjau permintaan ini dan tentukan persetujuan."}
            {canAdvance && `Ubah status pengiriman ke "${nextStatusMap[delivery.status]}".`}
          </div>
      ...
      <Card className="overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {filtered.length === 0 && <tr><td colSpan={6}><EmptyState text="Belum ada pergerakan stock." /></td></tr>}
          </tbody>
        </table>
        </div>
      </Card>
          ...
          {fields.map((f) => (
            <div key={f.key} className={f.fullWidth ? "sm:col-span-2" : ""}>
              ...
              {f.type === "select" ? (
                ...
                <input value={form[f.key]} onChange={(e) => setForm({ ...form, [f.key]: e.target.value })} placeholder={f.placeholder || f.label} className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-600" />
              )}
            </div>
          ))}
          {error && <div className="sm:col-span-2 bg-red-50 border border-red-100 text-red-700 text-xs rounded-lg px-3 py-2">{error}</div>}
          <div className="sm:col-span-2 flex justify-end gap-2">
            <GhostButton onClick={() => { setShowForm(false); setForm(emptyForm); setError(""); }}>Batal</GhostButton>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-xs text-gray-400 bg-gray-50/60 border-b border-gray-100">
              ...
              {fields.filter((f) => f.key !== "status").map((f) => <th key={f.key} className="px-5 py-3 font-medium">{f.label}</th>)}
              <th className="px-5 py-3 font-medium">Status</th>
              <th className="px-5 py-3 font-medium"></th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((it) => (
              <tr key={it[idField]} className="border-b border-gray-50 last:border-0">
                ...
                {fields.filter((f) => f.key !== "status").map((f) => <td key={f.key} className="px-5 py-3 text-gray-700">{it[f.key]}</td>)}
                <td className="px-5 py-3"><StatusBadge status={it.status} /></td>
                <td className="px-5 py-3"><button onClick={() => onToggle(it[idField])} className="text-xs font-medium text-emerald-800">{it.status === "Active" ? "Deactivate" : "Activate"}</button></td>
              </tr>
            ))}
...
export default function App() {
  ...
  const notifications = useMemo(() => {
    ...
    if (role === ROLES.SPV) {
      deliveries.filter((d) => ["Approved", "Preparing", "Shipped"].includes(d.status)).forEach((d) => {
